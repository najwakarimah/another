...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $no_order = ($paket_nasi_goreng_es_teh == 0 && $paket_ayam_bakar_es_teh == 0 && 
                 $paket_rendang_es_teh == 0 && $paket_gado_gado_es_teh == 0 && 
                 $paket_sate_ayam_es_teh == 0);

    // Check for confirmation submission
    if (isset($_POST['confirm_order'])) {
        // Process the order if confirmed
        // You can handle order processing here, like saving to a database, etc.
        echo "Pesanan Anda telah diterima dan sedang diproses!";
        exit;
    }
}
?>
...
<div class="container">
    ...
    <?php else: ?>
        ...
        <h3>Pesanan Anda akan segera diproses dan diantar ke alamat yang Anda berikan. Tim kami akan segera menghubungi Anda untuk konfirmasi lebih lanjut.</h3>

        <form method="POST">
            <input type="hidden" name="paket_nasi_goreng_es_teh" value="<?php echo $paket_nasi_goreng_es_teh; ?>">
            <input type="hidden" name="paket_ayam_bakar_es_teh" value="<?php echo $paket_ayam_bakar_es_teh; ?>">
            <input type="hidden" name="paket_rendang_es_teh" value="<?php echo $paket_rendang_es_teh; ?>">
            <input type="hidden" name="paket_gado_gado_es_teh" value="<?php echo $paket_gado_gado_es_teh; ?>">
            <input type="hidden" name="paket_sate_ayam_es_teh" value="<?php echo $paket_sate_ayam_es_teh; ?>">
            <input type="hidden" name="payment_method" value="<?php echo $payment_method; ?>">
            <button type="submit" name="confirm_order" class="button">Konfirmasi Pesanan</button>
        </form>
    <?php endif; ?>
</div>
